...
def VGG16_TRANSFER():
    ...
    vgg16 = VGG16(include_top=False, input_shape=(64, 64, 3)) #input_shape=(224, 224, 3)
    ...
    #don't show to avoid block here
    #plt.show()
...
if __name__ == '__main__':
    ...
    class_num = 800
    train_tfrecord_length = 55044
    test_tfrecord_length = 13760
    target_width = 64
    target_height = 64
    target_channel = 1

    train_idx, train_image, train_image_gray, train_label, train_label_one_hot = convert_from_tfrecords("datasets/tfrecords/train_" + str(target_width) + "_" + str(target_height) + ".tfrecords")
    print(len(train_idx))

    test_idx, test_image, test_image_gray, test_label, test_label_one_hot = convert_from_tfrecords("datasets/tfrecords/test_" + str(target_width) + "_" + str(target_height) + ".tfrecords")
    print(len(test_idx))
    train_tfrecord_length = len(train_idx)

    VGG16_TRANSFER()

    #stop here for training time measurement
